...
case "$platform" in
  ...
  *)
    ...
    echo "       Expected one of: linux-x64 linux-arm64 darwin-arm64 darwin-x64 win32-x64 win32-arm64" >&2
    exit 1 ;;
esac

# The server binary is built by server/default.do; depend on the
# per-triple output path rather than invoking cargo here.
redo-ifchange \
  package.json \
  ...
  src/extension.ts \
  README.md \
  "../../server/target/release/$triple/$binary"
...
cp "../../server/target/release/$triple/$binary" "$stage/out/bin/$binary"
