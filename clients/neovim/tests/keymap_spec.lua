...
describe('keymap.setup_for_buffer', function()
  ...
  it('installs gd → vim.lsp.buf.definition on status.jujutsu', function()
    local buf = open_named('.jj/badjuju/status.jujutsu')
    local found, m = has_buffer_map(buf, 'gd')
    assert.is_true(found, 'expected status.jujutsu map for gd')
    assert.are.equal(vim.lsp.buf.definition, m.callback, 'gd should invoke vim.lsp.buf.definition')
  end)

  it('installs gd → vim.lsp.buf.definition on log.jujutsu', function()
    local buf = open_named('.jj/badjuju/log.jujutsu')
    local found, m = has_buffer_map(buf, 'gd')
    assert.is_true(found, 'expected log.jujutsu map for gd')
    assert.are.equal(vim.lsp.buf.definition, m.callback, 'gd should invoke vim.lsp.buf.definition')
  end)

  it('installs gd → vim.lsp.buf.definition on diff.jujutsu', function()
    local buf = open_named('.jj/badjuju/diff.jujutsu')
    local found, m = has_buffer_map(buf, 'gd')
    assert.is_true(found, 'expected diff.jujutsu map for gd')
    assert.are.equal(vim.lsp.buf.definition, m.callback, 'gd should invoke vim.lsp.buf.definition')
  end)

  it('installs gd → vim.lsp.buf.definition on diff-change-<id>.jujutsu', function()
    local buf = open_named('.jj/badjuju/diff-change-abcdef.jujutsu')
    local found, m = has_buffer_map(buf, 'gd')
    assert.is_true(found, 'expected diff-change-*.jujutsu map for gd')
    assert.are.equal(vim.lsp.buf.definition, m.callback, 'gd should invoke vim.lsp.buf.definition')
  end)

  it('installs gd → vim.lsp.buf.definition on squash window', function()
    local buf = open_named('.jj/badjuju/squash/foo-bar.jujutsu')
    local found, m = has_buffer_map(buf, 'gd')
    assert.is_true(found, 'expected squash window map for gd')
    assert.are.equal(vim.lsp.buf.definition, m.callback, 'gd should invoke vim.lsp.buf.definition')
  end)

  it('does NOT install gd on describe.jujutsu', function()
    local buf = open_named('.jj/badjuju/describe.jujutsu')
    local found = has_buffer_map(buf, 'gd')
    assert.is_false(found, 'describe.jujutsu should not bind gd (no navigation context)')
  end)
...
describe('revision-scoped hotkeys send cursor-form', function()
  ...
  for _, case in ipairs({
    ...
    { key = 'D', server_command = 'badjuju.diff.commit' },
  }) do
    it(
